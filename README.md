...
当前 `VodMax.js` 版本：`5.4.7`。这是纯资源模块，重点优化了这些问题：
...
12. 动漫别名不再把具体作品散落硬编码在主流程里；优先使用 Forward 传入标题、TMDB 标题/翻译、Wikidata、TVMaze、Jikan 等动态别名，最后才使用集中维护的少量 VOD 常见译名兜底。`工房/工坊`、繁简异体这类通用写法差异仍会本地归一化。
13. 长篇动漫不只依赖少数作品名，高集数 TV 动画会进入全局集数匹配模式；本地兜底表只用于识别少数 VOD 源常见别名和污染过滤，不代表支持范围只限这些作品。
