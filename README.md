...
当前 `VodMax.js` 版本：`5.4.2`。这是纯资源模块，重点优化了这些问题：
...
6. 资源排序会弱加分 `1080p/HD/蓝光/高码/正片`，并降权 `TC/CAM/抢先/预告/解说/网盘分享页/疑似广告线路`。
7. 对长篇动漫按全局集数匹配，对第 0 季/OVA/特别篇要求有特别篇证据，避免串到普通季。
8. 如果 Forward 的资源模块没有传 `episodeName/airDate`，但传了 `tmdbId + season + episode`，会先用 TMDB 单集接口补全集标题和播出日期，再进入综艺期身份匹配。
